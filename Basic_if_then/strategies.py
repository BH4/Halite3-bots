...
def vacuum(game, ship_status, ship_destination, params):
    """
    The goal of this strategy is to suck up all the halite around when there
    isn't much left.
    """
    global num_dropoffs
    ...
    for ship in me.get_ships():
        ...
        if ship_status[ship.id] == "returning" and dist == 0:
            ship_status[ship.id] = "exploring"
        elif ship_status[ship.id] == "exploring" and ship.halite_amount == constants.MAX_HALITE:
            ship_status[ship.id] = "returning"
            dist = 0
        elif ship_status[ship.id] == "traveling" and dist < params.density_kernal_side_length:
            # Still need this in case this strategy activates while ships are traveling
            ship_status[ship.id] = "exploring"
            dist = 0
        ...
        if ship_status[ship.id] == "returning" and dist == 0:
            ...
            # Exploring moves should recalculate the destination every turn in
            # case the conditions change.
            destination = movement.vacuum_explore(ship, game_map, params)
            ship_destination[ship.id] = destination
